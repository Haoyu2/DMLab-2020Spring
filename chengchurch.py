...
def find_biclusters_np(matrix, n_of_bicluster=100, msr_threshold=300, alpha=1.2):
    ...
    for i in range(n_of_bicluster):
        ...
        rowsC, colsC, _ = single_deletion_node_np(
...
################################################################################
#   binarize matix by threshold
################################################################################
def find_biclusters_singleDeletion(matrix, n_of_bicluster=100, msr_threshold=300, alpha=1.2):

    matrixA = np.copy(matrix)
    biclusters = []
    rowsB = np.arange(matrix.shape[0])
    colsB = np.arange(matrix.shape[1])
    for i in range(n_of_bicluster):
        rowsC, colsC, msr = single_deletion_node_np(
            matrixA, rowsB, colsB, msr_threshold=msr_threshold)
        biclusters.append(Bicluster(rowsC, colsC, np.array([]), msr))
        matrixA = hide_bicluster_np(matrixA, rowsC, colsC)
    return biclusters


def binarize(mat, threshold):
    res = np.zeros(mat.shape, dtype = mat.dtype)
    thr = (mat.max()-mat.min()) * threshold
    res[mat>=thr] = 1
    return res
def binarize_01(mat, threshold):
    res = np.zeros(mat.shape, dtype = mat.dtype)
    res[mat>=threshold] = 1
    return res

def heatmap_bi(matrix, bicluster, name="Bicluster"):
    bicluster_matrix = get_bicluster(matrix, bicluster)
    fig, ax = plt.subplots()
    im = ax.imshow(bicluster_matrix)
